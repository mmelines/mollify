...
class MainCtl:
	...
	def __init__(self):
		...
		self.iter_lists = {}
		self.files = {"flask_dir__init__.py": {"stages": True,
										"filename": '__init__.py',
										"stage1": {"src": self.txt.init_txt, 
													"required": {"<APP_NAME>": config.APP_NAME}},
										"stage2": {"src": self.txt.init_txt}},
						"api__init__.py": {"stages": True,
										"filename": '__init__.py',
										"stage1": {"src": self.txt.api_init_txt, 
													"required": {"<APP_NAME>": config.APP_NAME}}},
						"main__init__.py": {"stages": True,
										"filename": '__init__.py',
										"stage1": {"src": self.txt.main_init_txt, 
													"required": {"<APP_NAME>": config.APP_NAME}}},
						"main.py" : {"stages": False,
										"filename": 'setup.py',
										"src": self.txt.main_txt,
										"required": {}},
						"api.py" : {"stages": False,
										"filename": 'setup.py',
										"src": self.txt.api_txt,
										"required": {}},
	...
	def iter1(self):
		...
		steps = [flask_main["__init__.py"], 
					...
					self.dirs["main"]["__init__.py"],
					self.dirs["main"]["main.py"],
					self.dirs["api"]["__init__.py"],
					self.dirs["api"]["api.py"]]
		for step in steps:
	...
	def assess_filetree(self):
		complete = {"flask_dir": {"__init__.py": {"found": False,
							...
							"main": {"main.py": {"found": False,
										...
										"__init__.py": {"found": False,
															...
															"type": "file",
															"extension": ".py"},
										"relative_path": "/<APP_NAME>/main"},
							"api": {"api.py": {"found": False,
															"complete": False,
															"name": "api",
															"exists_iter": 1,
															"complete_iter": 2,
							...
							"templates0": { "errors": {"found": False,
											...
											"pages": {"found": False,
															...
															"complete_iter": 2,
															"type": "dir"},
											"relative_path": "/<APP_NAME>/main/templates"},
							"templates1": { "errors": {"found": False,
															"name": "errors",
															"complete": False,
															"exists_iter": 1,
															"complete_iter": 2,
											...
											"pages": {"found": False,
															...
															"complete_iter": 2,
															"type": "dir"},
											"relative_path": "/<APP_NAME>/api/templates"},
							...
							"errors0": {"relative_path": "/<APP_NAME>/main/templates/errors"},
							"forms":	{"relative_path": "/<APP_NAME>/templates/forms"},
							"forms0":	{"relative_path": "/<APP_NAME>/main/templates/forms"},
							...
							"layouts0":	{"main.html": {"found": False,
															"name": "main",
															"complete": False,
															"exists_iter": 1,
															"complete_iter": 2,
										...
										"form.html": {"found": False,
															...
															"type": "file",
															"extension": ".html"},
										"relative_path": "/<APP_NAME>/main/templates/layouts"},
							...
							"pages0":	{"index.html": {"found": False,
															...
															"type": "file",
															"extension": ".html"},
											"relative_path": "/<APP_NAME>/main/templates/pages"}}
		...
		if self.has_dirs == False:
			for dirname in complete.keys():
				...
				for item in content:
					if item in complete[dirname[0]].keys():
						complete[dirname[0]][item]["found"] = True
				# include file instructions
				for item in complete[dirname[0]].keys():
					if isinstance(complete[dirname[0]][item], dict):
						if complete[dirname[0]][item]["type"] == "file":
							file_obj = complete[dirname[0]][item]
							filename = file_obj["name"] + file_obj["extension"]
							if file_obj["name"] == "__init__":
								filename = dirname[1] + filename
							if filename in self.files.keys():
								if filename == "__init__.py":
									this_init = dirname[1] + filename
									file_obj["info"] = self.files[this_init]
								else:
									file_obj["info"] = self.files[filename]
							final_path = path + "/" + file_obj["name"]
							final_path += file_obj["extension"]
							file_obj["path"] = final_path
							dir_obj[item] = file_obj
						else:
							dir_obj[item] = complete[dirname[0]][item]
							final_path = path + "/"
							final_path += complete[dirname[0]][item]["name"]
							dir_obj[item]["path"] = final_path
					else:
						dir_obj[item] = complete[dirname[0]][item]
				# change default name for project_root and flask_dir
				if dirname[1] in ["project_root", "flask_dir"]:
					dir_obj["role"] = dirname
					if dirname[1] == "flask_dir":
						self.dirs[config.APP_NAME] = dir_obj
					else:
						self.dirs[dirname[1]] = dir_obj
				else:
					self.dirs[dirname[1]] = dir_obj
			pprint(self.dirs)
			self.has_dirs = True
